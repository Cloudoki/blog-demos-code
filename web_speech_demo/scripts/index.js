...
function startSpeechRecognier () {
  ...
  recognizer.onspeechstart = function () {
    // detected sound that seem to be speech
      ...
      try {
        ...
        setTimeout(function () {
          if(!state.started && state.listening) {
            scroller.click();
            showAlert('Web Speech API seems to not be working. Check if you gave permission to access the microphone or try with another browser.');
          }
        }, 3000)
      } catch(ex) {
        console.log('Recognition error: ' + ex.message);
        showAlert('Failed to start recognizer.');
      }
    } else {
...
function autoScroll (speech) {
  ...
  if (typeof speech === 'string' || speech instanceof String) {
    ...
    if (speech.indexOf('up') > -1) {
      showNotification('Scrolling up...')
      window.scrollTo({
        top: currentHeight - 250,
        behavior: 'smooth'
      })
    } else if (speech.indexOf('down') > -1) {
      showNotification('Scrolling down...')
      window.scrollTo({
        top: currentHeight + 250,
        behavior: 'smooth'
      })
    } else if (speech.indexOf('top') > -1) {
      showNotification('Scrolling top...')
      window.scrollTo({
        top: 0,
        behavior: 'smooth'
      })
    } else if (speech.indexOf('bottom') > -1) {
      showNotification('Scrolling bottom...')
      window.scrollTo({
        top: pageHeight,
        behavior: 'smooth'
      })
...
function init () {
  ...
  if (SpeechRecognition === null) {
    showAlert('Web Speech API is not supported.');
  } else {
